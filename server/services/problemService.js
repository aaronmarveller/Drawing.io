...
// let problems = [     //constant should be named capital
//   {
//     "id": 1,
//     "name": "Staight line",
...
const getProblems = function() {
    ...
    return new Promise((resolve, reject) => {
      ProblemModel.find({}, (err, problems) => {  
        //put the error first, to avoid forgetting write errors
        if(err) {
          reject(err);
        } else {
          resolve(problems);
...
const getProblem = function(idNumber) {
    ...
    return new Promise((resolve, reject) => {
      ProblemModel.findOne({id: idNumber}, (err, problem) => {
        /*
        id: idNumber: 
        id means id in the database, 
        idNumber means the parameter of the function
        */
        if(err) {
          reject(err);
        } else {
